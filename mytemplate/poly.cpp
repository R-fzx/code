...
void init(int n = N, bool dont_calc_factorials = true) {
  ...
  if (!dont_calc_factorials)
    for (*finv = *fact = i = 1; i < n; ++i) fact[i] = (u64)fact[i - 1] * i % mod, finv[i] = (u64)finv[i - 1] * inv[i] % mod;
  t = min(n > 1 ? lg2(n - 1) : 0, 21), *w2 = 1, w2[1 << t] = PowerMod(unity, 1 << (21 - t));
...
void inv(int deg, pvec a, pvec b) {
  ...
  for (len = 0; 1 << len < deg; ++len) {
    NTT_init(len + 1), memcpy(B1 + (n >> 1), a + (n >> 1), n << 1), DIF(b, B2), DIF(B1, B3);
...
void div_mod(int A, int B, pvec a, pvec b, pvec q, pvec r) {
  if (A < B) {
    memcpy(r, a, (A + 1) << 2), memset(r + (A + 1), 0, (B - A) << 2);
    return;
  }
  int Q = A - B, i, l_ = Q ? lg2(Q) + 1 : 0;
  NTT_init(l_);
  for (i = 0; i <= Q && i <= B; ++i) B4[i] = b[B - i];
  memset(B4 + i, 0, (n - i) << 2), inv(i = Q + 1, B4, B5), std::reverse_copy(a + B, a + (A + 1), B4), NTT_init(++l_), memset(B4 + i, 0, (n - i) << 2), memset(B5 + i, 0, (n - i) << 2), mul(2 * Q, B4, B5, q), std::reverse(q, q + (Q + 1)), memset(q + i, 0, (n - i) << 2);
  if (!B) return;
  NTT_init(lg2(2 * B - 1) + 1);
  for (i = 0; i <= Q && i < B; ++i) B2[i] = b[i], B3[i] = q[i];
  memset(B2 + i, 0, (n - i) << 2), memset(B3 + i, 0, (n - i) << 2), mul(2 * (B - 1), B2, B3, r), memset(r + i, 0, (n - i) << 2);
...
void mul(vector &a, vector &b, vector &ret) {
  ...
  if (!A && !B) {
    ret.EB((u64)a[0] * b[0] % mod);
    return;
  }
  NTT_init(lg2(A + B) + 1), memcpy(B1, a.data(), (A + 1) << 2), memset(B1 + (A + 1), 0, (n - A - 1) << 2), memcpy(B2, b.data(), (B + 1) << 2), memset(B2 + (B + 1), 0, (n - B - 1) << 2), DNTT(B1), DNTT(B2);
  for (int i = 0; i < n; ++i) B1[i] = (u64)B1[i] * B2[i] % mod;
  IDNTT(B1), ret.assign(B1, B1 + (A + B + 1));
}
// Differential
void diff(int deg, pvec a, pvec b) {
...
void dif_quo(int deg, pvec a, pvec b) {
  ...
  int i, len = lg2(deg - 1);
  inv((deg + 1) / 2, a, B4), NTT_init(len + 1), memset(B4 + (n >> 1), 0, n << 1), DIF(B4, B2), diff(deg, a, B1), memcpy(B3, B1, n << 1), memset(B3 + (n >> 1), 0, n << 1), DIF(B3);
  for (i = 0; i < n; ++i) B3[i] = (u64)B3[i] * B2[i] % mod;
  DIT(B3, b), *b = *b * iv % mod;
  for (i = 1; i < n >> 1; ++i) b[i] = b[n - i] * iv % mod;
  memset(b + (n >> 1), 0, n << 1), DIF(b, B4), DIF(a, B3);
...
void exp(int deg, pvec a, pvec b) {
  ...
  pvec c = B6;
  assert(!*a);
  if ((*b = 1, deg <= 1) || (b[1] = a[1], deg == 2)) return;
  memset(b + 2, 0, i = 8 << lg2(deg - 1)), memset(c, 0, i), memset(B1, 0, i), *c = 1, neg(c[1] = b[1]);
...
inline void register_g(pvec g) {
  for (int i = 1; 1 << (i - 1) <= lim; ++i) NTT_init(i), DIF(g, gn + (1 << i));
}
...
void recursion(int id, int L, int R, const vector &poly) {
  ...
  if (L + 2 == R) return tmp_.EB((poly[n] + (u64)poly[n - 1] * g[rp].back()) % mod),
...
#define term(k) (u64) a[i + k] * b[i + k]
  for (; i + 15 < len; i += 16) {
    ans = (ans + term(0) + term(1) + term(2) + term(3) + term(4) + term(5) + term(6) + term(7) + term(8) + term(9) + term(10) + term(11) + term(12) + term(13) + term(14) + term(15)) % mod;
  }
#undef term
