    ...
    if (K==1) {
        for(register int i=1;i<=n; i++) {
            for (int j = 1; j < a[i] % 10; j++) {
                for (int j2 = a[i] % 10; j2 < 10; j2++) d.insert(make_pair(j, j2));
            }
            for (int j = 1; j <= b[i] % 10; j++) {
                for (int j2 = b[i] % 10 + 1; j2 < 10; j2++) d.insert(make_pair(j, j2));
            }
        }
        for (int i = 1; i <= 9; i++) {
            print(i);
            if (d.count(make_pair(i, i + 1)) || d.count(make_pair(i + 1, i))) puts("");
        }
        puts("");
    } else if (n <= 50) {
        for (int i = 1; i <= n; i++) {
            for (int j = a[i]; j <= b[i]; j++) {
                ...
                for (int k = 0; k <= min(getbits(j) - 1, K - 1); k++) {
                    ...
                    for (int q = 1; q <= 9; q++) {
                        num = j;
                        if (p == q) continue;
                        if (d.count(make_pair(p, q))) continue;
                        num = num - p * pow10k + q * pow10k;
                        if (!check(num)) d.insert(make_pair(p, q));
                    }
                }
            }
        }
