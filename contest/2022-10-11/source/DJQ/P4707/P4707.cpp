...
inline void Choose(int len) {
    ...
    int Cnt = People[len].L + Present[len] * 10;
    a[len] = 1, tot++, P *= (double)(Cnt) / 100.0, Choose(len + 1);
    if(Cnt) P /= (double)(Cnt) / 100.0; tot--;
    Cnt = 100 - People[len].L - Present[len] * 10;
    a[len] = 0, P *= (double)(Cnt) / 100.0, Sum += People[len].B, Choose(len + 1);
    if(Cnt) P /= (double) Cnt / 100.0; Sum -= People[len].B;
}
inline void Search(int len, int x) {
    if(len == n + 1) {  
        Total = 0, P = 1, Choose(1), Answer = max(Answer, Total);
        return ;
    }
