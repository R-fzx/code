...
// #define ONLINE_JUDGE
  ...
  c[1] = 1;
  rep(i, 1, n) {
    f[i] = (f[i - 1] * 10 % kM1 + s[i] - '0') % kM1, fs[i] = (fs[i - 1] + f[i]) % kM1, c[i + 1] = c[i] * 2 % kM1;
  }
  LL p = 1;
  rep(l, 1, n - 2) {
    p = p * 10 % kM1, ans = (ans + (fs[n] - fs[l - 1] - fs[n - l] * p % kM1 + kM1 + kM1) % kM1 * c[n - l - 1] % kM1 + (f[l] + (f[n] - f[n - l] * p % kM1 + kM1) % kM1) % kM1 * c[n - l - 1] % kM1) % kM1;
  }
  printf("%lld", (ans + f[n] + f[n - 1] + f[n] - f[1] * p % kM1 * 10 % kM1 + kM1) % kM1);
#ifdef TIME
  fprintf(stderr, "\nTIME: %dms", clock());
#endif
  return 0;
