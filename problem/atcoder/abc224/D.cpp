...
#define openFile(f) freopen(f".in","r",stdin),freopen(f".out","w",stdout),freopen(f".log","w",stderr)
...
int B() {
  for (R({0, 1, 2, 3, 4, 5, 6, 7, 8, 0}, 0); h <= t; ++h) {
    if (q[h].first == s) return q[h].second;
    rep(i, 1, 9) {
      if (!q[h].first[i]) {
        rap(j, e[i]) {
          Ai y = q[h].first;
          swap(y[i], y[j]), R(y, q[h].second + 1);
        }
      }
    }
  }
