...
int h, w, n, f[kN], hm[kN], wm[kN], d[kN];
...
#define ONLINE_JUDGE
  ...
  cin >> h >> w >> n;
  rep(i, 1, n) {
    cin >> e[i].r >> e[i].c >> e[i].v, d[i] = i;
  }
  sort(d + 1, d + n + 1, [](int i, int j) { return e[i].v > e[j].v; });
  int p = 0;
  rep(i, 1, n) {
    if (e[d[i]].v != e[d[p]].v) {
      rep(j, p, i - 1) {
        cmax(hm[e[d[j]].r], f[d[j]]), cmax(wm[e[d[j]].c], f[d[j]]);
      }
      p = i;
    }
    f[d[i]] = max(hm[e[d[i]].r], wm[e[d[i]].c]) + 1;
  }
  rep(i, 1, n) {
    printf("%d\n", max(0, f[i] - 1));
  }
