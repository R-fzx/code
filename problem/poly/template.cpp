...
const int kN = 3e5 + 1, kM = 998244353, kG = 3, kiG = 332748118;
int n, iv[kN << 1], _a[kN << 1], _b[kN << 1];
...
void NTT(int a[], bool o) {
  ...
  for (int l = 2; l <= n; l <<= 1)
    for (int i = (_l = l >> 1, u = pow(kG, (kM - 1) / l), 0); i < n; i += l)
      for (int j = (w = 1, i); j < i + l; ++j) x = 1LL * w * a[j + l] % kM, a[j + l] = (a[j] - x + kM) % kM, a[j] = (a[j] + x) % kM, w = 1LL * w * u % kM;
  if (o) return;
  for (int i = (reverse(a + 1, a + n), 0), iv = pow(n, kM - 2); i < n; ++i) a[i] = 1LL * a[i] * iv % kM;
}
void mulf(int a[], int b[], int c[], int na, int nb) {
  init(na + nb), cpy(c, a, na), cpy(_a, b, nb), clr(c, na, n), clr(_a, nb, n), NTT(c, 1), NTT(_a, 1);
  for (int i = 0; i < n; ++i) c[i] = 1LL * c[i] * _a[i] % kM;
  NTT(c, 0);
}
void invf(int a[], int b[], int m) {
  init(m), clr(b, 0, n), b[0] = pow(a[0]);
  for (int l = 2; l <= n; l <<= 1) {
