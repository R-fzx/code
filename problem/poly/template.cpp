...
void expf(int a[], int b[], int m) {
  static int _a[kN], _b[kN];
  init(m << 1), clr(_a, 0, n), clr(_b, 0, n), clr(b, 0, n), b[0] = 1;
  for (int l = 2; l <= m << 1; l <<= 1) {
    lnf(b, _a, l), init(l), clr(_a, l, n), cpy(_b, a, l), NTT(b, 1), NTT(_a, 1), NTT(_b, 1);
    for (int i = 0; i < n; ++i) b[i] = (1LL * b[i] * (1LL - _a[i] + _b[i]) % kM + kM) % kM;
    NTT(b, 0), clr(b, l, n);
  }
}
void sqrf(int a[], int b[], int m) {
  static constexpr int i2 = pow(2);
  static int _a[kN];
  init(m), clr(_a, 0, n), lnf(a, _a, m);
  for (int i = 0; i < n; ++i) _a[i] = 1LL * _a[i] * i2 % kM;
  expf(b, _a, m);
}
}  // namespace Poly
